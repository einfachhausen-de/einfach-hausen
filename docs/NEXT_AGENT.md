> **V0-R1-FIX 2026-09-24 (Branch `v0/r1-example-honesty`, NICHT gemergt):** R1 erledigt — Fischer-Porträts gelöscht, gesichtslose `TradeAvatar` + sichtbares `ExampleBadge` (site.tsx), alle Fantasiefirmen/-namen und Sterne-Ratings raus; zusätzlich `craftsman-at-work.png` („Schmidt“-Jacke) und `pro-workshop.png` (Mercedes/Kennzeichen) neutral neu erzeugt. Gelb G1–G4, G6–G9 umgesetzt (G5 bleibt Designautorität). Gates in v0: tsc 0, eslint 0, test:public-site PASS; `design:check` meldet erwartete Änderung an geschütztem `site.tsx` → Re-Seal durch Subagent. Details: `docs/agent-handoffs/v0-r1-fix-20260924.md`. Nächste Aktion: Subagent Re-Seal + build + public-nav + E2E/Visual (OCI-VM) → Merge → Jeremy-Abnahme.
