...
> **NAECHSTE AKTION (genau eine):** Betreiber-Schritte aus **Punkt 9** in `docs/EXTERNAL-BLOCKERS.md` umsetzen (Resend-Domain verifizieren, SPF/DKIM setzen, `MAIL_FROM` umstellen). Danach versendet das System E-Mails **ohne Codeaenderung**; heute ist der Versand fail-closed und im Healthcheck sichtbar. Parallel zu beantworten: **Punkt 10** (Push bauen? — haengt direkt am Store-Risiko Guideline 4.2) sowie die Betreiber-Vorleistungen aus **Punkt 11** (Apple-Konto, Bundle-ID, IAP-vs-Stripe, CocoaPods + JDK/SDK). Offen: #91 (visuelle Abnahme Jeremy), #12, #11, #10; geschlossen: #33, #132, #127–#130. **Kein Design-Re-Seal ohne ausdrueckliche Designautoritaet** (`docs/brand/design-authority-requests.md`). **Betriebshinweis:** `free_disk_percent` auf der VM steht bei 14 — Backup-Rotation im Auge behalten.
