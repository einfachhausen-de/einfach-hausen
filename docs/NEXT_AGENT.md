> **DOKUMENTEN-DATEIBROWSER 2026-09-21 — BRANCH feat/documents-file-browser-20260921:** Die bisherige Dokumentenseite wird zu einem echten Dateibrowser konvergiert: Werkbank-Sidebar links, Ordner und Dateien gemeinsam in der Mitte, breitere Vorschau rechts, Listen-/Rasterumschalter, Suche, Sortierung, mobile Fokusführung. Daten bleiben auf den aktuellen Eigentümer begrenzt; private PDF-/Bildendpunkte behalten ihre Autorisierung und werden nur same-origin eingebettet. Handoff: docs/brand/workspace/DOCUMENT-BROWSER-HANDOFF.md; Taskplan: docs/TASKPLAN_DOCUMENT_BROWSER_20260921.md. Gezielte Tests 6/6 und isolierte Chromium-Prüfung grün. **Nächste Aktion:** vollständige Repo-Gates und authentifizierte Browserprüfung auf Online-Host; danach Jerry-Visumabnahme. Kein Deployment/keine Baseline-Änderung in dieser Welle.

> **DEPLOY 2026-09-21 LIVE (ed7c3aa, Prod-Head verifiziert):** Zwei GitHub-Issue-Wellen abgeschlossen. **#127–#130 (Affiliate/Vergleiche)** als `45a6af3`: zentrale Partnerkonfiguration `src/lib/affiliate.ts` (fail-closed, leerer Start), serverseitige Ausleitung `GET /api/affiliate/[category]` (401 ohne Session, nie ein Ziel vom Client, UUID-only Klickreferenz, `Referrer-Policy: no-referrer`), Bereich „Vergleichen & Wechseln“ in `/app/contracts?tab=vergleichen`, getrennte Versicherungswege, 24 Kontrakttests (`npm run test:affiliate`), Datenklassifizierung `affiliate_clicks` in `docs/privacy/DATA_INVENTORY.json`. **#132 (Eigentuemer dauerhaft kostenlos)** als `5289d1e`: Owner-Membership-/Package-Monetarisierung entfernt (Checkout-Actions, Stripe-Webhook-Zweige, `/api/memberships`, `/api/packages`, `src/lib/packages.ts`, Flag `pilot_cohort_open`, Seeding); Tabellen und die eine historische `subscriptions`-Zeile (ohne `stripe_subscription_id`) bleiben erhalten; `/app/plans` → `/app/settings`, `/pilotphase` → `/preise`. Binding-Docs auf das kostenlose Modell umgestellt (`ed7c3aa`: `docs/PRODUCT_VISION.md`, `README.md`). Committete Merge-Konfliktmarker in `AGENTS.md`, `docs/brand/mature-reference/SOURCE.md`, `docs/brand/final-documents/SOURCE.md` als Vereinigung beider Historien aufgeloest (kein Inhalt verworfen). Gate-Haertung: `scripts/release-gate.mjs` loescht vor dem Typecheck das generierte `.next/types` — der Deploy war mit 14/15 abgebrochen, weil veraltete generierte Typen auf die geloeschten Routen zeigten. Verifikation: Release-Gate 15/15 lokal und auf der OCI-VM, `test:visual` 72/72 (Baseline `tests/visual-baselines/preise.png` bewusst neu, inhaltlicher Umbau), Prod `/api/health` 200, `/login` 200, `/api/affiliate/strom` 401, `/api/memberships/success` 404. Issues #127–#130 und #132 mit Evidenz kommentiert/geschlossen, #14 und #87 fachlich abgeglichen.
> **NAECHSTE AKTION (genau eine):** Designautoritaets-Entscheidung zu `docs/brand/design-authority-requests.md` einholen (DA-2026-09-21-01: `design:check` geschuetzte Dateien + Rohwert-Debtbasis; DA-2026-09-21-02: `design:deadcss` Budgets und `stateClasses 20 > 16`). Alle offenen GitHub-Issues (#91, #33, #12, #11, #10) sind extern blockiert; ohne diese Entscheidung ist keine autonom erledigbare Aufgabe offen. **Kein Re-Seal ohne ausdrueckliche Designautoritaet** — `design:check` und `design:deadcss` bleiben bis dahin rot, der Release-Gate (15/15) liest sie nicht.
