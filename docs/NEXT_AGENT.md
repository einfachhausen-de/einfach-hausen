> **STARTSEITEN-Layout Mobil/Desktop (2026-09-23, `839bc05`):** Betreiber-Order aus Screenshots: (1) Die vier Rail-Kennzahlen (Auftraege/Angebote/Ansprechpartner/Termine) laufen ueber einen Grid-Wrapper `styles.railStatGrid` in `src/app/app/page.tsx` — Desktop-Rail wie bisher eins pro Zeile, mobil (Uebernahme in den Mittelbereich, <=1023.5px) zwei nebeneinander; nicht zurueck auf margin-gestapelte Einzellinks (der Rand-Abstand der Kacheln lebt nicht mehr, Grid-Gap traegt). (2) Die Profil-Fortschrittskarte (Angaben x von 4) ist auf ausdruecklichen Wunsch KOMPLETT raus, mobil wie Desktop, inkl. Berechnung — nicht wieder einbauen. (3) Schnellaktionen <=600px: Primarkkarte full-bleed, darunter Beratung starten und Tarife vergleichen nebeneinander (Medienblock an `quickGridThree` in `eigentuemer-start.module.css`). Hinweis: `/app/preview` zeigt dieselben Kacheln aus eigener Quelle (`preview/page.tsx`) und bleibt bei Stapelung — bewusst nicht angefasst.
