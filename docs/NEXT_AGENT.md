> **AUDIT-ABSORPTION 2026-09-19 (74a3406, main, gepusht):** Alle P0/P1 aus `docs/REPO_AUDIT_2026-09-18.md` sind erledigt. Letzte Welle: P1-8 (letzte 10 Owner-Seiten + 4 Provider-Seiten auf WerkbankRahmen, AppShell unter src/app zu 100% weg), P1-18 (`/onboarding/pro` Legacy-Wizard stillgelegt), P2-26 (`/welcome`+`/role` sind serverseitige Redirects, T-0168-konform), P0-7 (Bewertungs-Meldung kommt wieder auf der Partnerdetailseite an — vorher wurde die Erfolgsmeldung vom `/app/partners`-Redirect geschluckt), visueller Kontrast-Bug „Ansprechpartner finden" (fast schwarzer CTA durch Legacy-`.btn.primary` im Werkbank-Scope → versiegelter `EHButton`), Profilbalken-Regression (data-fill ohne CSS-Regel → immer 0 px). P2-22 (8 tote Dateien), P1-17 (310 tote CSS-Regeln), P2-24 (53 Icon-Exports), P1-19 (Schema-Baseline repariert: Fresh-DB == Dev-DB) ebenfalls erledigt. **Verifiziert KEINE Bugs:** P0-6 `/pro`-Sperrbildschirm ist korrektes Verhalten; P2-25 Anfrage/Auftrag ist begrifflich richtig. Vollstaendige Doku: Handoff Kap. 14/15/16.

> **⚠️ FROZEN — LAYOUT-RICHTUNGSWECHSEL (Operator, 2026-09-19):** Die linke Seitenleiste wird die HAUPTNAVIGATION (aufklappbar, shadcn sidebar-07); der Header fuehrt kuenftig nur Suche/Notifications/Tools. Niemand ausser der Sidebar-Welle darf ab sofort anfassen: `shell.module.css`, `nav-config.ts`, `werkbank-rahmen.tsx`, `owner-menu`, `bottom-nav`. Untracked auf der Platte (NICHT von ausserhalb committen!): `src/app/dashboard/`, `src/components/app-sidebar.tsx`, `nav-main/nav-projects/nav-user/team-switcher`, `src/components/ui/*`, `.orca/drops/`, `package.json`-Aenderungen.

> **OFFEN (alles nicht-blockierend):** P2-28 — 8 native `<input type="file">` ueber `EHInput` zeigen rohen Browser-Text; `EHFileInput` fehlt im **versiegelten** `packages/eh-design` → Designautoritaet. P2-23 — dead CSS in `globals.css`/`design-system.css` → versiegelt → Designautoritaet. `docs/preview-werkbank-2026-09-18.html` ist VOR dem Sidebar-Umbau bewusst nicht erneuert worden (sonst sofort veraltet).
