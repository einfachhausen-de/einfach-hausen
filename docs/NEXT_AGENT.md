...
> | `test:public-nav` | **ROT** | Browser-Aufloesung war macOS-only und ist in `0ad0083` behoben (Suite lief auf der VM gar nicht). Danach echte Layout-Zusicherung rot: weiter Auth-Trust-Panel 468 px, gefordert >= 820 px (`public-navigation-e2e.mjs:96`). **Designautoritaet** — nicht abschwaechen. |
> | `test:auth-design` | **ROT** | 4 Vertragsverletzungen in `src/components/auth-v2/LoginForm.tsx` gegen `scripts/auth-edition-contract.mjs`: verbotenes `lucide-react`, verbotenes `className=`, fehlendes `getSupabase`, fehlendes `signInWithPassword`. Ursache ist der Umbau auf Server-Actions (`loginAction`/`registerAction`) und lucide-Icons. **Das passt zur T-0168-Regel** (Client ist keine Security Boundary, autorisiert wird serverseitig) — der Waechter kodiert also die Vor-T-0168-Architektur. **Nicht abschwaechen, nicht umbauen:** entweder ist LoginForm abgewichen oder der Vertrag braucht eine autorisierte Aktualisierung. **Autoritaetsentscheidung.** |
> | `test:e2e` | **ROT** | Veraltet nach der Designsystem-Migration (`ef4c3e6`, 32 Seiten auf `packages/eh-design`) und den Auth-Route-Redirects (`74a3406`). Behoben in `515d696`/`0ad0083`: Auth-Routen (jetzt Redirect-Zusicherungen, staerker als vorher), Registrierung ueber `#btn-submit-register` statt Button-Text, mehrdeutige Ueberschrift, Partnerkarten-Selektor. **Naechster Abbruch:** Provider-Navigation — der Provider-Shell rendert `EHWorkspaceFrame` und nicht mehr das alte `<details class="mobile-menu">`. |
> | `test:e2e:architecture` | **ROT** | Gleiche Klasse: wartet auf `**/pro` nach der Registrierung, das der alte Ablauf nicht mehr ausloest (`e2e-architecture.mjs:146`). |
> **Konsequenz fuer den naechsten Agenten:** Ein gruenes Release-Gate bedeutet NICHT, dass die Abnahme gruen ist. Vor jedem "fertig" die Suiten einzeln laufen lassen. Die verbleibende Re-Basierung von `test:e2e`/`test:e2e:architecture` ist genau **T-0006** und ist grosse, klar umrissene Arbeit (Selektoren + Texte nach der Designsystem-Migration). **Zwei Punkte darin brauchen die Designautoritaet** (Trust-Panel-Breite, LoginForm-Vertrag) und duerfen nicht durch Anpassen der Zusicherungen geloest werden.
