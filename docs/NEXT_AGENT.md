> **SIDEBAR-WELLE 2026-09-22 (gebaut, lokal grün, NICHT gepusht/deployed):** Owner-Sidebar nach Betreiber-Vorgabe umgebaut (Jerry-Screenshots 2026-09-21). **Neu:** Unten „Zentrale“ mit Ansprechpartner über Benachrichtigungen direkt über dem Profil-Button; Profil/Hilfe & Kontakt/Einstellungen aus der Sidebar raus ins Avatar-Menü (NavUser: Identität + Profil + Hilfe & Kontakt + Einstellungen + Abmelden); Avatar-Menü öffnet sich nach oben (`side="top"`, vorher `right` → wirkte abgerissen/zu weit rechts). **Nebenbei behoben:** Owner-Icons waren verschoben (stale `WalletCards` in `OWNER_ACCOUNT_ICONS` → Hilfe zeigte Karten-Icon); `OWNER_ACCOUNT_ICONS` entfällt, Ansprechpartner-Icon/Label kommen aus `ownerAreas` (kein Doppel-Eintrag: oben aus `Navigation` gefiltert, `nav-config` unverändert → BottomNav/Breadcrumbs/Active-Logik intakt; Provider-Sidebar unverändert). E2E-Zusicherung in `scripts/e2e.mjs` auf neue Struktur nachgezogen (Sidebar: Bereiche + `/messages` + `/notifications`, kein Profil/Hilfe/Settings-Duplikat; Avatar-Menü-Einträge + Links geprüft). Gates lokal: tsc 0, eslint 0, `design:check` CONSISTENT, `next build` ok. GitNexus detect-changes: Risk CRITICAL (erwartet: Sidebar hängt an jeder Route; reine Menübestückung, keine Logik). Geändert: `src/components/app-sidebar.tsx`, `src/components/nav-user.tsx`, `src/components/nav-projects.tsx` (nur Kommentar), `scripts/e2e.mjs`. **Nächste Aktion:** `test:e2e` + `test:visual:update` auf OCI-VM (Sidebar-Baselines ändern sich beabsichtigt), dann visuelle Jeremy-Abnahme (Pflicht vor Deploy), dann Push/Deploy.
