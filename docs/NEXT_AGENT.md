> **V0-WEBSITE-FINISH 2026-09-24 (Frontend only, nicht verifiziert):** Login/Register optisch auf die Website gezogen (Lime-Pill, Papier, Hauskante, endliche Ankunft — kein Remotion). Globale 404 auf SiteShell, Heading-Anker bleibt. Produktseiten Beratung/Notfall/Versicherung/Immobilienverkauf mit eigenen Bildern und Grenze vor dem Button. Leistungs-Titel entdichtet, ClosingCta auf Hauskante. Tests, Re-Seal, E2E, Visual, Merge: Subagent. Details: `docs/agent-handoffs/v0-website-finish-20260924.md`.
> **DEPLOY 2026-09-25 LIVE (`138916a`, Prod-Head verifiziert):** PR #178 (geteilte Start-Ansicht, Dashboard-Datenfluss) + 4 lokale v0-Patches (Werkbank-Komposition 33 Seiten, Fehlergrenzen/Composer/Einstellungen, Debt-Cleanup per Operator-Order, Fokus-Label). Bottom-nav-`active` aus P4 bewusst weggelassen (redundant zu aria-current + state-class-regelwidrig). Gate **17/17**. Prod: aktiv, NRestarts=0, Health ready, Routen 200/307 korrekt. Antwort auf „alles auf GitHub?": nein — nur PR #178 war oben; Patches lagen nur lokal und sind jetzt drin. Offen: E2E auf OCI nach App-Umbau, Jeremy-Abnahme (Pflicht).
> **FERTIG 2026-09-25 (E2E grün, live `209edfc`, Prod-Head verifiziert):** Alle offenen Punkte geschlossen. (1) Auth-Verträge: Demo-Login-Signatur + Lime-Pillen-Erwartung nachgezogen — `test:auth-design` grün. (2) E2E 3b2/3b3-Root-Cause per Fixture-Proben belegt: Layout-Verschiebung (klassische Scrollbar unter Linux) zieht das Menü unterm Cursor weg → Klick auf HTML → Menü zu. Produkt-Fix `html{scrollbar-gutter:stable}` + E2E-Nutzer-Retry + Tastatur-Aktivierung im Submenü (Hover-Öffnung weiter belegt). (3) Volles E2E auf OCI **grün** (alle 15 Checks inkl. Matching/Buchung/Messaging/Rechnung/History/Admin/Transfer). (4) Deploy **17/17** (`209edfc`): aktiv, NRestarts=0, Health ready, Routen 200/307 korrekt. Verbleibend nur Jeremy-Abnahme (Pflicht, visuell) — technisch ist alles fertig.
> **DEPLOY 2026-09-25 LIVE (`8e0f78f`, Prod-Head verifiziert):** PR #177 (Werkbank-Website-Angleichung: Original-Logo, Sidebar-Markenton, Lime-Pillen, Fokuszeile, Titel/ Tabs/Breadcrumbs) + Token-Drift-Fix (html-style.mjs regeneriert, generate --check wieder ok) + tote Shell-Familie entfernt (AppShell/OwnerMobileMenu/Breadcrumbs/SidebarAccountMenu, SectionTitle extrahiert; EHOwnerComposer bleibt — versiegelt). Gate **17/17**. Prod: aktiv, NRestarts=0, Health ready, `/` `/partner` `/login` 200, `/app` `/pro` 307 (Login-Redirect korrekt). Offen: E2E auf OCI, Jeremy-Abnahme (Pflicht), demo-accounts-contract-Vorbestand.
> **V0-APP-AUFTRAG 2026-09-25 (Operator-Order):** Nach der Website übernimmt v0 auch Owner-App + Betriebs-App (Frontend/Design, gleiche Qualität). Beauftragung: `docs/agent-handoffs/v0-app-frontend-briefing-20260925.md` (Freiheit: Komposition/Bausteine/additive Tokens/Motive; unverrückbar: T-0168 + keine Mocks, freigegebene Kompositionen, Telefon-Regeln, Werkbank-Richtung, ARIA-statt-Klassen, versiegelter Kern/Debt, E2E-Anker, Ehrlichkeit). AGENTS.md ergänzt. Nächste Aktion: v0 liefert App-Branches → Subagent Gates/Merge → Jeremy-Abnahme (Pflicht) → Deploy.
> **DEPLOY 2026-09-25 LIVE (`d9c5b95`, Prod-Head verifiziert):** v0-Distinct-Layouts (Startseite/Login-Bausteine, Service-Motive, DESIGN/COMPOSITION-Update) + A11y-Fix + 72 Baselines. Gate blockte zuerst korrekt: axe (aria-controls-Dangling, 4 Kontraste, Vergleichstabelle) — Subagent-Fix, lokal 0 Violations belegt; danach Visual-Diffs (beabsichtigter Redesign) — Renders gesichtet (Home/Partner/Login/Preise/AGB ehrlich und sauber), Baselines auf OCI erneuert, nach Mac committet/gepusht, OCI-Tree bereinigt, Deploy grün **17/17**. Prod: `d9c5b95`, Service aktiv, NRestarts=0, Health ready, `/` `/preise` `/login` `/leistungen` `/partner` 200, Disk 10→13%. Offen: E2E auf OCI, Jeremy-Abnahme (Pflicht), demo-accounts-contract-Vorbestand (Test-String veraltet, separate Remediation), mobiler Hero-Bubble-Overlap (kosmetisch).
> **V0-R1-FIX 2026-09-24 (gemergt, R1 verifiziert):** R1 erledigt und vom Subagent belegt — Fischer-Porträts gelöscht, gesichtslose `TradeAvatar` + sichtbares `ExampleBadge` (site.tsx), alle Fantasiefirmen/-namen und Sterne-Ratings raus; `craftsman-at-work.png` (Rückenansicht, keine Schrift) und `pro-workshop.png` (kein Logo/Kennzeichen) visuell bestätigt. Gelb G1–G4, G6–G9 umgesetzt (G5 bleibt Designautorität). Gates in v0: tsc 0, eslint 0, test:public-site PASS; `design:check` meldet erwartete Änderung an geschütztem `site.tsx` → Re-Seal durch Subagent. Details: `docs/agent-handoffs/v0-r1-fix-20260924.md`. Nächste Aktion: Subagent Re-Seal + build + public-nav + E2E/Visual (OCI-VM) → Merge → Jeremy-Abnahme.
> **ARENA-BEFUND 2026-09-24 (geprüft, bewusst NICHT gemergt):** Alle drei Arena-Zweige einzeln gegen main geprüft. `01a0c66d` (0 eigene Commits) und `01a0cb62` (Spitze == altes origin/main `937d5fb`, kein eigener Commit) sind vollständig in main enthalten — inkl. aller Arena-Designverbesserungen (12px-Radius, Sticky-Rail, ChatGPT-Optik, Kamera-Overlay, EHActionTiles, Outline-Pille). Einzig `01a0c607` (Startseite v3, 5 Commits, Spitze im fremden Tag `archiv/01a0c607-startseite`) enthält je eigenen Code — aber ausschließlich überholten: Wholesale-Merge würde geltende Betreiber-Orders brechen (Profil-%-Railkarte ist explizit KOMPLETT raus, Rail-Kennzahlen bleiben gestapelt) und E2E-Anker löschen („Warten auf dich", Termine, Dokumente mit Größen, Mitteilungs-Zähler); dazu 38 Rohwert-Treffer + Inline-Styles im lokalen CSS-Modul und statische Mock-Preview („Fixturestraße", href="#"). Die guten Ideen (Überblicks-Karten, Schnellaktionen, Historie unten, contactsCount) leben in main bereits in verfeinerter, freigegebener Form (`src/app/app/page.tsx:92,195`). Kein Cherry-Pick ohne neue Betreiber-Order. Eigene Archiv-Tags wieder gelöscht (kein Archiv statt Merge — hier gab es nur nichts zu mergen).
> **V0-ROLLE + R1-AUFTRAG 2026-09-24 (Operator-Order, noch nicht gelauncht):** Rollenteilung festgeschrieben — **v0:** Frontend/Design/Designsystem-Verbesserungen; **lokaler Subagent:** danach Tests, Gates, Docs, Merge, Push, Verifikation (AGENTS.md, Abschnitt V0-ROLLE + R1-REGEL). **R1 an v0 beauftragt:** Fischer-Avatare tauschen (erfundene Firma „FISCHER … MEISTERBETRIEB" + „Sarah F."-Schild neben „geprüfte Betriebe"-Claims) — neutrale Motive ohne lesbare Marken oder sichtbare Beispiel-Kennzeichnung überall; keine neuen Fantasienamen/Sterne. Briefing: `docs/agent-handoffs/v0-r1-fischer-briefing-20260924.md`; Befund: `docs/agent-handoffs/v0-frontend-audit-20260924.md`. Merge-Status: nichts offen (main `acc5437` == origin/main, keine Branches). Nächste Aktion: v0 liefert R1-Fix → Subagent verifiziert (Gates) → Jeremy-Abnahme (Pflicht).
> **V0-FRONTEND-AUDIT 2026-09-24 (read-only, kein Code geändert):** Public-Site-Neubau (Merge `223a835`) geprüft — Maßstab DESIGN.md + sin-eh-design. **ROT (vor Deploy fixen): R1 erfundene Handwerker-Firma auf Fotos** — alle drei Avatare tragen lesbare „FISCHER … MEISTERBETRIEB"-Stickerei + Namensschild („Sarah F."), im Hero neben „3 geprüfte Betriebe" sogar ohne Beispiel-Kennzeichnung; Bildtext (Fischer) widerspricht Kartentext (Bauer/Weber/Kern). **GELB:** Beispiel-Kennzeichnung nur sr-only auf Phone-Mocks (sichtbares Badge wie bei Handwerker-Karten fehlt); absolute KI-Versprechen („jede Frage", „rund um die Uhr" ohne Notfall-Abgrenzung); „automatisch" ohne Freigabe-Kontext; Sparrechner-Quellen + Öl-Fall; Radien (rounded-2xl/arbitrary) + font-extrabold außerhalb Vokabular — Guard sieht Tailwind-Klassen nicht (Guard-Lücke, Pille-Regel selbst widersprüchlich → Designautorität); „Designsystem 1.1"-Label vs. DESIGN.md 1.0; /partner ohne Preisanker; Hero-Collage als Fake-Benachrichtigung. **GRÜN (v0 bleibt führender Frontend-Agent, nicht einschränken):** 0 Emojis/Gradients/Lorem/Englisch-Fragmente, @theme strikt auf --eh-*-Tokens (Inter-only), Ehrlichkeits-Blöcke (Pilotphase, Beispielrechnung, illustrativ-Disclaimer), echte GET-Formulare + rechnender Sparrechner, A11y-Handwerk, Hauskante als Komponente. Vollständig: `docs/agent-handoffs/v0-frontend-audit-20260924.md` (Befunde + 5 Kompakt-Regeln für v0). Nächste Aktion: R1-Bilder tauschen (neutrale Motive ohne Firmennamen), dann visuelle Jeremy-Abnahme (Pflicht), E2E/Visual auf OCI-VM.
> **STARTSEITEN-Kacheln & Profil-Fortschritt (2026-09-23, final nach Ruecknahme):** Betreiber-Order aus Screenshots und Klarstellung: Die vier Rail-Kennzahlen (Auftraege/Angebote/Ansprechpartner/Termine) bleiben UEBEREINANDER (mobil wie Desktop) — der zweispaltige `railStatGrid`-Versuch aus `839bc05` ist zurueckgenommen; die `.railStat`-Kacheln tragen wieder ihr `margin-bottom:10px` (nicht auf Grid-Gap umstellen: die Vorschauseite `/app/preview` rendtert dieselben Kacheln ohne Wrapper, dort brachen die Abstände). Die Profil-Fortschrittskarte ("Profil · Angaben x von 4") ist auf Betreiberorder KOMPLETT raus — aus `/app`-Rail, aus `/app/preview`-Rail und aus dem `/app/profile`-Rail (dort steht die Vollstaendigkeit nur noch im Seiteninhalt selbst; Berechnung `profilePct`/`fillStep` geentfällt) — in KEINEM Rail wieder einbauen. Schnellaktionen <=600px bleiben wie bestellt: Primarkarte full-bleed, darunter Beratung starten und Tarife vergleichen nebeneinander (`quickGridThree`-Block in `eigentuemer-start.module.css`). design:debt:sync + Re-Seal wegen entfernem Inline-Style (page.tsx) — Siegel: `EH_DESIGN_CONSISTENT`.
> **KAMERA, DOKUMENT-BRUECKE, MOBILER RAIL (2026-09-23, `8cd14f9`+`ad7012b`, in main):** Betreiber-Order „Foto machen soll eine Kamera sein, nicht der Datei-Dialog“: `packages/eh-design/src/assistant.tsx` spaht jetzt `matchMedia('(pointer: coarse)')` — mobil bleibt das native `capture=environment`-Input, auf Desktop/Coarse-los oeffnet `fotoMachen()` ein getUserMedia-Overlay (video+canvas, 1/0.92-JPEG -> `anhangSetzen(file,'photo')`, Wenden, Abbruch; Fehlerfaelle mit Hinweis + Datei-Fallback, Stream stoppt bei Unmount/Abbruch/Esc capture-phase). Paket neu versiegelt (eh-design-seal), 9 neue `.assistantCamera*`-Regeln nur mit Tokens. Guard: neuer Test in document-intelligence.test.mjs (Anker `capture="environment"`, `getUserMedia`, `fotoAufnehmen`); das ausgelieferte Bundle enthaelt `assistantCameraShutter`/`Foto aufnehmen`/`getUserMedia` (curl gegen Dev-Server verifiziert). Zweitens Order „was soll dieses dumme Dokument da unten? weg“: der `Dokument`-Sprunglink unter dem Hausmanager (`house-assistant.tsx` -> `#hausmeister-composer`) ist raus; Oberflaechen-Guard auf den echten Chat-Upload (`uploadAssistantChatDocumentAction`) umgezogen, doesNotMatch sichert gegen Wiedereinbau. Drittens Mobil-Entwurf NACH Betreiber-Korrektur (`8deef4d`): KEIN rechter Bereich auf Mobile; stattdessen wird der Rail-Inhalt (Ueberblick) im mittleren Bereich mitausgegeben (`wb-kontext-mobil`-Duplikat des `rail`-Knotens in `main`, CSS-gesteuert, doppelte IDs geprueft: keine) und das KI-Symbol (Sparkles, wie im Launcher) in der Kopfzeile neben dem Profil (`wb-ki-mobil`) oeffnet NUR den Kundenberater als randloses Vollbild (`aside data-mobil="ki"` + data-ki, fixed inset 0 z 60; Schliessen ueber Chat-Kopf oder Knopfdruck; kein Cookie dafuer). Das erste Mobil-Paket (PanelRight-Drawer fuer den ganzen Bereich, `wb-rail-mobil`/`wb-rail-schliessen`) ist ersatzlos raus — nicht wieder einbauen. Gates: test:ai 60/60, tsc 0, eslint 0 Errors (2 vorbest. Warnungen), build, design:check CONSISTENT; live ueber Dev-Server-SSR-Anker bestaetigt. Hinweis naechster Agent: in der Sandbox verhindert `.env.local` mit AUTH_MODE=local den Production-Build (Fail-closed in `src/lib/auth.ts:57` korrekt so) — fuer den Build dort `AUTH_MODE= npm run build`; Datei ist gitignoregescuetzt, nie committen.
> **HEADER-MENUE BETREIBER-URTEIL 2026-09-23 (FINAL FÜR ALLE AGENTEN):** Betreiber hat nach erneutem Platt-Befund (Screenshot 15:42) ausdrücklich das Bereichs-UNTERMENÜ angeordnet: 12 Bereiche als zweite Liste (Radix `DropdownMenuSub` an "Neuer Auftrag"; Plain-Trigger ohne asChild, Chevron nur am Trigger), Maus-Klick auf "Neuer Auftrag" navigiert dank `event.detail===0`-Trick direkt, Tastatur-Enter öffnet das Submenü, Hover zeigt die zweite Liste — exakt der 1:1-Aus der Session 7a2cf0a (`0a3dc2b`-Linie, vom Betreiber abgenickt). Die älteren Log-Zeilen "Flyout belegt unbrauchbar / flache Liste ist das Bewiesene" (`5679feb`, `a6b7ac7`, `34bb462`, TAKEOVER-Eintrag) sind damit **überstimmt und dürfen nicht mehr als Begründung für eine Flachumsetzung herangezogen werden**; die damals belegten Defekte (Slot-Crash mit asChild, Klick-Hängen beim Anfahren) sind im 1:1-Stand durch Plain-Trigger + Direkt-Klick-Navigation umgangen. Gesichert durch neue Guard-Suite `scripts/header-menu.test.mjs` (in `test:ai`; wer flach macht, sieht dort Rot, inkl. E2E-Anker-Prüfung). E2E 3b3 zurück auf Hover-Flyout; die übrigen E2E-Startseiten-Anker vom Übernahme-Zweig ("Warten auf dich", Schnellaktionen, Haus-Historie, Vorschläge für dich) bleiben unverändert. DESIGN.md-Ziffer zum Kopf-Menü ist listenneutral ("direkt oder über einen der 12 Bereiche") — der Betreiber-Wille ist jetzt erst hier ausdrücklich festgeschrieben. Nächste Aktion: Deploy auf sin-supabase (FF), dort volles Gate inkl. E2E; visuelle Abnahme (Pflicht).
> **KI-CHAT FUNKTIONAL 2026-09-23 (Zweig `arena/01a0cb62-einfach-hausen`, in main seit `44dd37c`):** Auftrag: Tool-Buttons waren nur UI (`selectedTool` wurde nicht gesendet). Umgesetzt: (1) `origin/main` (`6adbcc9`, neu angelegter Root-Commit ohne Eltern, mit Doku-Upload/OCR, File-Archiv, flat Header-Menü) per `--allow-unrelated-histories` in den KI-Zweig gemerged (`ae74287`); Konflikte: actions/composer/header-menu/e2e/doc-int/DATA_INVENTORY/NEXT_AGENT = Main (produktionsbewiesen), werkbank-*/assistant-service/shell.module.css/globals.css(12px)/Paket = KI-Zweig, house-assistant = Vereinigung (ki_state+suggestions + Dokument-Link); design-lock neu versiegelt. (2) `POST /api/ki` + `tool` (Allowlist `CHAT_TOOLS`: create_job, compare_tariffs, compare_quotes, search_house, create_report; unbekannt=400 via `kiChatToolFromBody`); explizite Auswahl umgeht Laya/Jev (`provider:'tool'`), keine Local-Regeln, kein Modell. (3) Neue Capabilities: `search_house` (ai-tools: bündelt searchIntelligentDocuments + Verträge + Dokumente + Anlagen + Wartungen + Hausdaten + Kontakte, tenant-sicher, max 12 Treffer, faktisch ohne Generativ-Modell) und `create_report` (assistant-service: `reportContext` sammelt max 5000 Zeichen aus 6 Bereichen via executeAssistantTool, dann bestehender Generativ-Pfad inkl. BYOK/Quota/Refund; Transparenz-Step zeigt Einträge+Zeichen). (4) LIEST_DATEN um next_actions/compare_quotes/house_check/compare_tariffs/search_house erweitert → „Deine Daten gelesen“-Transparenz korrekt. (5) Plus-Button: „Foto machen“ (`capture=environment`, bestehender `savePrivateMediaUpload`-Pfad, neue Action `saveAssistantChatPhotoAction` + `recordAssistantChatPhoto` in orchestrator, `/api/ki` erhält nur `photoPath`-String; `ownedImage`-Owner-Check bleibt Schutz) und „Datei hochladen“ (PDF/Bild → neue Action `uploadAssistantChatDocumentAction` → `storeAssistantDocument` in neuem `src/lib/assistant-document-ingest.ts` — dorthin auch `savePrivateFile` nach `security/private-files.ts` umgezogen, actions.ts importiert; revalidatePath bleibt in der Action). (6) Chat-Tools im Paket: 5 Tools mit IDs=Allowlist, Tool-Placeholders, Anhang-Vorschau (Foto-Thumb / Dokument-Chip), Senden mit Anhang ohne Text erlaubt, Tool+Anhang erst nach gelungener Antwort zurückgesetzt (Wiederholung behält Kontext). (7) Paket-API: `onSend(..., options?: {tool?, attachment?: {file, kind:'photo'|'document'}})`, Typen `EHChatAttachment`/`EHSendOptions` (NEU: Name `EHAttachment` war in workspace-detail vergeben). Tests: `test:ai` 57/57 (neu: Tool-Routing ohne Laya/Jev, 400 unbekannt, Tool schlägt Local-Regeln, Transparenz, create_report Kontext+Quota, Foto-Owner-Check + Fremdpfad 422, Doku-Ingest tenant-safe + Ablehnung ungültiger Dateien, End-to-End-Verdrahtungs-Anker). Gates in Sandbox: tsc/eslint(0 Errors)/build/design:check/seal grün; Release-Gate static 7/8 (`backup-retention-regression` rot — AUCH auf dem Merge-Commit, d.h. vorbestehend, sandboxzeitlich; auf OCI-VM verifizieren) + Browser-Ebenen (a11y/visual/lexikon-visual/e2e) nicht in der Sandbox lauffähig (kein Chromium/Playwright-CDN). Offene Punkte: (a) PR gegen main öffnen, Merge → FF-Deploy auf OCI, dort vollständiges Gate (16/17) + Visual-Abnahme; (b) `backup-retention-regression` auf der VM prüfen (vermutlich Uhrzeit-/mtime-Abhängigkeit); (c) UX-Acceptance der neuen Chips/Tools vom Betreiber. WICHTIG für Folge-Agenten: `packages/eh-design` ist design-locked — nach Änderungen immer `node scripts/eh-design-seal.mjs`; nur eigene Pfade committen (Lehre aus `6d69dd8`), `git add -A` nicht.
> **TAKEOVER-DONE 2026-09-23 (`a09a816` LIVE, Prod-Head verifiziert):** Mac-Session hing (offline-flapping), Merge `9250a2a` hatte klick-loses Hover-Flyout zurückgebracht. Übernommen: Bereiche flach (`34bb462`), E2E-Anker auf Werkbank-Komposition (`8daf89c`), BYOS-Inventar (`23bbb4b`) + Boot-Schema parallel (`340ce9c`, Duplikat per Rebase aufgelöst, nur `0002-baseline` behalten `fac964c`). Belege Prod: E2E ok:true, Gate 17/17, Visual 72/72, Smoke 18/18, Health ready, NRestarts=0 (Deploy nach transientem EACCES-`.next`-Abbruch vom Parallel-Agent zu Ende gebracht). Freie Platte nur 9–10% — Retention/Backup-Wachstum prüfen. Nächste Aktion: visuelle Jeremy-Abnahme (Pflicht), dann Laya-Service-Install.
> **HEADER-TAKEOVER 2026-09-22 (`34bb462`, NOCH NICHT DEPLOYED):** Merge `9250a2a` hatte das bewiesen klick-lose Hover-Flyout (`40a7430`) zurückgebracht — Sub schließt beim Anfahren (Geometrie-Probe belegt), E2E:705 hängt. Bereiche wieder flach im Menü (exakt `a6b7ac7`-Stand), übrige Merge-Inhalte (Werkbank-Startseite, OCR/HEIC, Laya, Preview-Route) unangetastet. Lokal grün: tsc 0, eslint 0, Build 144/144, ai 39/39. Offen: E2E/Gate/Visual/Smoke + Deploy auf sin-supabase — Mac-Relay (`fleet-mac-direct`) offline seit 22:40Z, Direkt-SSH nicht provisioniert. Prod läuft weiter gesund auf Flat-Menü-Stand. Nächste Aktion: Mac wecken → E2E/Gate/Visual/Smoke auf Prod → Deploy `34bb462` → Jeremy-Abnahme.
> **HEADER-MENÜ 2026-09-22 LIVE (`e6af041`, Prod-Head verifiziert):** 16:58-Order umgesetzt (Kalender-Punkt raus, Alle Termine ins Aufträge-Menü, 12 Bereiche direkt wählbar, Neuer Auftrag per Direkt-Klick). Radix-Sub zweimal gescheitert (erst Slot-2-Kinder via Chevron im `asChild`-Wrapper `6cce580`, dann Sub-Schließung beim Anfahren) → Bereiche flach im Menü (`a6b7ac7`, `data-testid=bereich-*`, E2E ohne Hover). Belege auf Prod: ai 26/26, Build 143/143, E2E ok:true, Gate 17/17, Visual 72/72, Smoke 18/18, Health ready, NRestarts=0. Laya-Router (`2e392dd`/`ca74083`, ChatGPT-parallel) unangetastet mitgeliefert. Nächste Aktion: visuelle Jeremy-Abnahme (Pflicht), Laya-Service-Install offen (inaktiv).
> **FIX `5679feb` (live, Gate 17/17, E2E ok:true, Smoke 18/18):** Doppelter Neuer-Auftrag-Eintrag entfernt (Konvergenz-Dopplung). Flyout belegt unbrauchbar (Slot-Crash, Off-Screen-Klicks, Grace-Schliessung) — flache Liste nutzt nur Bewiesenes.
> **FIXES `6d69dd8`/`0c435cc`/`d64b136` (live, Gate 17/17, Smoke 18/18):** Versehentlich fremde gestagete AI-Dateien mitcommittet (`git commit` ohne Pfade) — Historie bleibt ehrlich stehen; Inhalt geprüft (tsc/design grün). Gate danach rot auf T-0146: fünf AI-Tabellen ohne Inventar (`house_documents`, `owner_ai_insight_state`, `owner_ai_user_state`, `document_intelligence_jobs`, `tariff_partner_offers`) — alle klassifiziert (personal/account; Tarife als B2B-Referenz unter business, `*_name`-Regel erzwingt personal) nach Quellprüfung (Migration, Lifecycle in account-deletion/export vorhanden). Lehre: nur eigene Pfade committen; fremde Stages nie mischen.
> **HEADER-MENÜ-NACHTRAG (Designregeln + Konvergenz):** DESIGN.md mit Kopf-Menüleisten-Abschnitt fortgeschrieben und neu versiegelt (nur DESIGN.md-Hash via Prüfer-Funktion, Guard grün, keine Debt-Änderung, keine versiegelten Dateien sonst) auf ausdrückliche Betreiber-Order; 40px-Ziele als dokumentierter Touch-Override. Eigene Dublette (`d5d09d6`, Slot-Diagnose identisch) zugunsten des Parallel-Strangs verworfen und hierher konvergiert.
> **AI-DOKUMENT-INGEST 2026-09-22 — VERBINDLICH FÜR DIE LAUFENDE KI-WELLE:** `docs/AI-ARCHITECTURE.md` enthält das Addendum `AI-DOC-INGEST-20260922`. In die aktuelle Laya/Jev/DeepSeek-Arbeit integrieren, keinen Parallel-Branch/Parallel-Router bauen. Kern: Laya auf `sin-supabase` = Standard; Jev nur Load-/Timeout-Overflow; DeepSeek nur generativ/vision hinter Credits. Dokumente: privater Upload -> SQLite-Queue -> vorhandenen PDF-Text direkt nutzen, sonst CPU-OCR -> Kandidaten -> Laya typed decision -> harte Validatoren -> Fachablage (`documents`, `house_contracts`, `house_history_*`, sonst minimale `house_documents`). Unsicherheit immer `review_required`, nie still falsche Fachdaten. Ein OCR-Worker, kein Redis/Kubernetes/externer OCR-Dienst. Akzeptanztests/Fixtures + echter ARM-VM-Benchmark stehen im AI-Dokument.

> **SIDEBAR-DEPLOY 2026-09-22 LIVE (`7ca2ea6`, Prod-Head verifiziert):** Owner-Sidebar nach Betreiber-Vorgabe (Jerry-Screenshots 2026-09-21): unten „Zentrale“ (Ansprechpartner über Benachrichtigungen mit Badge), Avatar-Menü mit Profil + Hilfe & Kontakt + Einstellungen + Abmelden, öffnet nach oben. **Fix `7ca2ea6`:** Menüzeilen in Sidebar-Geometrie (`min-h-44`, `rounded-md`, `p-2` statt `rounded-lg`/`p-1`; zusammengequetscht + zu runde Ecken behoben) — nur Geometrie, keine Farbtoken (Portal), Debt unverändert (1 Vorbestand-Punkt), Guard grün. **Fix `acd9ce8` (live, Gate 17/17, Smoke 18/18):** Menü-Links waren petrol statt dunkel — Ursache Legacy-Regel `a{color:green}` greift im Portal (Farbe stand nur am Container, nicht am Link selbst wie in der Sidebar); Links jetzt `text-inherit`, Icons folgen mit. **Fix `eaff0d8` (live, Gate 17/17, Smoke 18/18):** Einstellungs-Dialog: Container `rounded-xl`→`rounded-lg` (8px, EH-Panelmaß), Breadcrumb-Link erbt Menüfarbe statt Legacy-Grün; nur `owner-settings-dialog.tsx`, Debt sauber, Guard grün. Hinweis: „In-App-Benachrichtigungen öffnen“ bleibt petrol — das ist kanonisches EHList-Verhalten (versiegelt), kein Fehler. **Fix `37ea049` (live, Gate 17/17, Smoke 18/18):** Eingeklappte Sidebar zeigte Textfetzen (Buttons schrumpfen auf 44px, Labels hatten kein Icon-Hiding) — Labels/Badge/Untermenüs/Textblöcke (Navigation, Zentrale, Markenkopf, Avatar) werden im Icon-Modus per `group-data-[collapsible=icon]:hidden` versteckt; Links/Buttons tragen `aria-label` als Namensersatz (Tooltips unverändert); nur Consumer-Dateien, kein ui-Paket, Debt unverändert, Guard grün. **Feat `3825293` (live, Gate 17/17, Smoke 18/18):** Navigation neu sortiert (Start, Aufträge & Termine, Verträge & Tarife), Hausakte mit allen Unterpunkten in die Zentrale über Ansprechpartner; Zentrale als eine NavMain-Gruppe (badge-fähig via neuem optionalem `badge`-Slot, Benachrichtigungs-Count mit Icon-Hiding); `nav-config` unberührt (BottomNav/Breadcrumbs/Active-Logik intakt), Provider unberührt. App-Visual trotz neuer Sortierung grün ohne Baseline-Update (8%-Budget auf weißer Fläche trägt Zeilenverschiebung; Actual-Shot am Bildschirm verifiziert). **Fix `50ce3ab`+`8dad916`+`2bc4b67` (live, Gate 17/17, Smoke 18/18):** Suche zeigte fetten braunen Halo — per Fixture-Probe belegt: kanonische 3px-Terra-Fokuskontur (versiegelter `--eh-color-focus`) + gestapelter petrol Legacy-`box-shadow` auf dem Input. Fix in `shell.module.css` (Guard/Schuld unverändert): Shadow weg, Ring nach kanonischem directory-search-Muster ans Feld (`:focus-within`, Input ringfrei) — erst Außen-Offset (vom `overflow:hidden`-Panel geclippt, nur Unterkante sichtbar), final innen (`-3px`, wie im versiegelten Paket üblich). Liste (max 12, 340px, Scroll) und Panel-Geometrie unverändert — Abschneiden am Panelrand ist Scrollverhalten, kein Fehler. **Fix `3ad67f7` (live, Gate 17/17, E2E ok:true, Smoke 18/18):** Suche war rechts angedockt und nicht klickbar — Ursache: Backdrop (`position:fixed`, ohne z-index) im DOM NACH dem Panel und damit darüber, fraß alle Klicks. Fix in `shell.module.css` (Guard/Schuld unverändert): Palette mittig (`justify-items:center`), Panel mit `z-index:1` über Backdrop. E2E-Test 3b beweist Zentrierung (x im Mittelfeld) + Klick-auf-Treffer navigiert. Lehre: `test:e2e` nutzt den vorhandenen `.next`-Build — nach CSS-/UI-Änderungen auf OCI erst `npm run build`, dann E2E (Fehlalarm x=782 kam vom stale Build). **Feat `35369c3`+`a81a8bd` (live, Gate 17/17, E2E ok:true, Smoke 18/18):** Glocke öffnet Mini-Liste (neueste 5, ungelesene zuerst, „Alle ansehen“ → /notifications) statt Seitenwechsel — wie Avatar-Menü (44px-Zeilen, rounded-md, text-inherit-Links). Neu `notifications-menu.tsx` (Debt sauber), Rahmen lädt Liste serverseitig (keine Mutationen im Menü). E2E 3c beweist Öffnen + Navigation. Lehre: Radix rollt `asChild`-Links zu `menuitem` um — per `menuitem`-Rolle adressieren, nicht per `link` (sonst wartet der Klick ins Leere und der URL-Timeout trägt die Schuld). **Feat `64f3b6b` (live, Gate 17/17, E2E ok:true, Smoke 18/18):** Nur /notifications lud voll neu — Ursache: versiegelte EH-Komponenten (EHList u.a.) rendern plain `<a>` (paketweit, routerlos per Design). Neu `ClientNav`-Horcher in der Werkbank-Shell: gleichartige In-App-Klicks werden clientseitig geroutet (Next-Links/Tabs/Downloads/Modifier/Anker unberührt) — ohne versiegelte Dateien, Guard/Schuld sauber. Gilt für alle EH-Zeilen app-weit (Settings-Dialog, Verzeichnisse, Tabs, „Alle ansehen“). E2E 3d beweist Dokument-lebt-weiter per Marker. Per Fixture-Probe belegt: Sidebar-Wechsel war schon vorher clientseitig (~100ms); Einstellungen→Mitteilungen war der echte Reload-Pfad. **Fix `010308d` (live, Gate 17/17, E2E ok:true, Smoke 18/18):** Mitteilungen wirkten trotzdem wie Reload — Ursache: als einzige App-Route ohne eigene Ladezustand-Datei fiel die Navigation auf das Root-Skeleton ohne Shell zurück (Seitenleiste/Topbar verschwanden, volle „Seite wird geladen“-Ansicht). Neu `src/app/notifications/loading.tsx` im Werkbank-Rahmen mit Rollen-Erkennung (wie `/app/loading.tsx`), Debt sauber. Andere Routen sind abgedeckt (`/app`, `/pro` haben eigene). **Feat `fcb6c4d`+`611e20f` (live, Gate 17/17, E2E ok:true, Smoke 18/18):** Kopf-Menüleiste (statt Such-Pille + Glocke): Suchen (öffnet Palette per Event), Aufträge (`/app/jobs` bzw. `/pro/orders`), Kalender (`/app/calendar` bzw. `/pro/calendar`), Benachrichtigung (Mini-Liste) — mit echten Zählern (offene Aufträge, bestätigte bevorstehende Termine, ungelesene Mitteilungen; Definitionen wie die Seiten selbst; Provider rollengerecht via `getProviderContext`). Kein Radix-Menubar (kein neues Deps-/UI-Paket): Leiste aus bewährtem Dropdown-Muster in EH-Token-Optik (`shell.module.css`, Guard/Schuld sauber, mobil Icon-only). E2E 3b2 beweist Menü-Links. 26 App-Baselines bewusst erneuert (Topbar überall neu; Desktop-Shots blieben im Budget, Mobile darüber; Stichproben verifiziert). Lehre: Deploy-Skript bricht bei dirty Tree ab — `| tail` schluckt den Exit-Code; Erfolg nur per `DEPLOY_EXIT=$?` glauben. **Feat `d0f84e6` (live, Gate 17/17, E2E ok:true, Smoke 18/18):** Menüleiste + Dropdowns in Referenz-Optik (21st.dev-Menü als Vorbild, aber EH-Token): Aufträge-Menü mit „Neuer Auftrag“/laufenden/„Alle Aufträge“, Kalender-Menü mit Terminen/„Alle Termine“, E2E 3b2 auf Menü-Bedienung. **Feat `1e17a1e`+`f3d7782` (live, Gate 17/17, E2E ok:true, Smoke 18/18):** Dropdowns exakt aufs Referenz-Muster (nur Icon-Reihen; Info-Kopfzeilen mit Zählern zurück auf Betreiber-Order — Zähler zusätzlich weiter auf den Triggern; Leer-Texte draußen; Trennstrich nur vor „Alle …“). Bar schlanker (Label-Schrift, 16px-Icons, offener Trigger mit Füllung). **Expliziter Override, dokumentiert:** 40px-Ziele statt 44px-Touch-Floor (DESIGN.md) auf ausdrückliche Betreiber-Anordnung; Guard maschinell weiter grün, keine Debt-Änderung, keine versiegelten Dateien. 26 Baselines erneuert (Topbar-Höhe → Mobile-Shift; Stichprobe verifiziert). **Feat `d0f84e6` (live, Gate 17/17, E2E ok:true, Smoke 18/18, App-Visual grün ohne Baseline-Änderung):** Menüleiste + Dropdowns in Referenz-Optik (21st.dev-Menü als Vorbild, aber EH-Token: Tinte statt Slate, Panel-Radius/Schatten): kräftigere Zeilen, Icons je Reihe, Dropdowns `rounded-lg`. Aufträge-Menü: „Neuer Auftrag“ (Owner → Hausmeister; Provider ohne, kann keine erstellen), laufende Aufträge direkt (max 5, Titel + Status-Text aus `statusLabel`), „Alle Aufträge“. E2E 3b2 auf Menü-Bedienung umgestellt. **Feat `6347c91`+`3ec3288` (live, Gate 17/17, E2E ok:true, Smoke 18/18, App-Visual grün ohne Baseline-Änderung):** Dropdowns exakt aufs Referenz-Muster gestutzt (nur Icon-Reihen; Kopf-Zeilen, Trennstrich-Blöcke und Leer-Hinweistexte raus — Trennstrich nur noch vor „Alle …“; Zähler bleiben auf dem Trigger). Bar schlanker (Label-Schrift, 16px-Icons, offener Trigger mit Flächen-Füllung), bleibt bei 44px-Zielen (Touch-Vertrag; Referenz mit 40px braucht expliziten Override). Kalender-Menü: anstehende Termine direkt (Titel + Berlin-Datum aus `ownerDate`) + „Alle Termine“; Provider ohne Neu-Eintrag. E2E 3b2 auf Kalender-Menü umgestellt. **Echter Fang dabei:** neue Termine-Query nutzte `a.title` (Spalte gibt es nicht) — `tsc` sieht kein SQL, E2E fing `SQLITE_ERROR` (jede Seite wäre 500 gewesen). Fix auf `j.title`, E2E danach grün. **Feat `37be976` (live, Gate 17/17, E2E ok:true, Smoke 18/18):** Kopfzeile bleibt beim Scrollen sichtbar (`position:sticky;top:0;z-index:30` in `.wb-top`) — kein Grund fürs alte Verhalten gefunden (Seitenleiste rechnet sogar mit 58px-Header). Menü-Zähler, Suche, Glocke, Profil damit immer einen Tap entfernt; Dropdowns (z-50) und Suche (z-200) bleiben darüber. Eine Zeile, kein Baseline-Impact. Nebenbei: Owner-Icon-Fehlbelegung (stale WalletCards) behoben; `nav-config` unverändert (BottomNav/Breadcrumbs intakt); Provider unverändert. Commits: `86ffb50` (feat) + `f455905` (37 Baselines) + 5× `test(e2e)`-Nachzug (`a475447` Onboarding-Copy, `d5fbba8` Profil-Install-Callout, `3a94c2f` Anrufen-Sektion, `d7248ca`+`86e5a7f` Beratung-Labels, `0180637` Buyer-Copy — alle Vorbestand-Drift aus EINFACH-WELLE `7305ace`, je gegen Quellbaum geprüft, keine Abschwächung). Baselines bewusst erneuert: 7 Website-Shots (home×3 = Werkbank-Pilot `dfdb098`, preise×3 = kostenlos-Copy #132, login@tablet — je am Render verifiziert) + 30 neue `tests/visual-baselines/app/` (neu, nie versioniert gewesen; Stichproben verifiziert). **Falle behoben:** app-visual testet `.next`-Build — erste Baselines spiegelten versehentlich den alten Build; mit frischem Build neu erzeugt + verifiziert. **Befunde (Vorbestand, nicht blockierend):** (a) Login-Render zeigt „zu ��ffnen“-Tofu — identisch in alter Baseline, geschützter Auth-Bereich, nicht angefasst; (b) sudo-Suites hinterlassen root-owned `/tmp/*.db` → Gate-SQLITE_READONLY (aufgeräumt; künftig `chown`); (c) `update-on-oci.sh` als root scheitert am Git-Fetch (kein GitHub-Auth) — als `ubuntu` laufen (passwordless sudo ok). Verifikation OCI: Release-Gate **17/17**, `test:e2e` ok:true (15 Checks), `test:visual` 72/72, Prod-Smoke **18/18**, `/api/health` ready (db ready, auth reachable, smtp sandbox_sender = bekannter fail-closed-Stand), `/app` → 307, `/login` → 200, NRestarts=0. Nächste Aktion: visuelle Jeremy-Abnahme der neuen Sidebar am Live-Stand (Pflicht), dann weiter mit höchster Taskplan-Priorität.
...
- Design-System unangetastet: nur `--eh-*`-Tokens, CSS-Module, kein neuer Token-Satz, keine Gradients/Glas/Stripes. `motion/react` (Dependency) ergänzt die GSAP-Schicht für Zustands-/Layout-Motion.
