...
> **KI/OCR OWNER-INTELLIGENCE 2026-09-22 LIVE (`786d560` + `05eb636` + `77942d4`, Prod verifiziert):** Laya läuft auf `sin-supabase` resident und loopback-only; 12 Eigentümer-Fähigkeiten, owner-scoped Tools, proaktive In-App-Hinweise, Tarifpartner-Grenze und Dokument-Intelligence sind gebaut. Dokumente: privater Upload → vorhandenen PDF-Text direkt lesen, sonst lokales Tesseract; HEIC/HEIF wird lokal per `heif-convert` normalisiert → genau eine Laya-`document_kind`-Choice → sichere Metadaten/„Bitte prüfen“. Echter Wegwerf-Smoke belegt Text-PDF→Laya, PNG→OCR→Laya und iPhone-HEIC→OCR→Laya; Garantieende wird als Erinnerungsdatum erkannt. Node-KI 35/35, Python-Service 3/3, Release-Gate 17/17, öffentlicher Smoke 18/18. Laya-Key konfiguriert; Jev/TypeSafe- und DeepSeek-Betreiberkeys fehlen weiter. Source of Truth: `docs/LAYA_OWNER_INTELLIGENCE.md` + `docs/AI_ROUTER.md`. Der KI/OCR-Produktionsnachweis endet bei `77942d4`; parallele Owner-UI-Commits können `main` danach weiterbewegen und sind separat zu releasen/verifizieren.
> **WICHTIG FÜR FOLGEAGENTEN:** ältere Zeilen in diesem Langzeit-Handoff, die „Laya-Service-Install offen/inaktiv“ oder das frühere `AI-DOC-INGEST`-Addendum als laufenden Plan bezeichnen, sind historisch und werden durch den Block oben ersetzt.
...
> **AI-DOKUMENT-INGEST 2026-09-22 — HISTORISCH, UMGESETZT:** Der damalige Plan ist durch die gebaute und getestete Implementierung ersetzt. Operative Source of Truth sind `docs/LAYA_OWNER_INTELLIGENCE.md` und `docs/AI_ROUTER.md`; das alte Addendum in `docs/AI-ARCHITECTURE.md` wurde bewusst entfernt, damit kein zweiter widersprüchlicher OCR-Plan bestehen bleibt.
