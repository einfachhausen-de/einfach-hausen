...
export function EHWorkspaceFrame({navigation,account,mobileMenu,notifications,context,children,homeHref,bottomNav,mainNav,rail}: {navigation:ReactNode;account:ReactNode;mobileMenu:ReactNode;notifications:ReactNode;context:string;children:ReactNode;homeHref:string;bottomNav:ReactNode;mainNav?:ReactNode;rail?:ReactNode}) {
 return <div className={s.workspace + (rail?" "+s.workspaceWithRail:"")}><EHSidebar homeHref={homeHref} navigation={navigation} account={account}/><main className={s.workspaceMain}><header className={s.workspaceTop}><div className={s.workspaceMobile}>{mobileMenu}<EHLogo href={homeHref}/></div>{!mainNav&&<span className={s.workspaceContext}>{context}</span>}<div className={s.workspaceTools}>{notifications}</div></header>{mainNav?<nav className={s.workspaceNavRow} aria-label="Hauptnavigation">{mainNav}</nav>:null}<div className={s.workspaceContent}>{children}</div>{rail?<aside className={s.workspaceRail} aria-label="Kontext dieser Seite">{rail}</aside>:null}</main><div className={s.workspaceBottom}>{bottomNav}</div></div>;
}
