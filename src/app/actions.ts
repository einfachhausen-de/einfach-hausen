...
export async function sendHausmeisterAction(fd:FormData){
  ...
  if(document){
    ...
    try{stored=await savePrivateFile(document,'house-documents');}
      ...
      redirect('/app/hausmeister?error=Das%20Dokument%20konnte%20nicht%20sicher%20%C3%BCbernommen%20werden.%20Bitte%20nutze%20PDF%20oder%20ein%20Bild%20bis%2012%20MB');
    }
    ...
    enqueueDocumentIntelligence({homeownerId:user.id,sourceType:'house_document',sourceId:documentId,storedPath:stored,originalName:title,mimeType:document.type});
    const analysis=await processDocumentIntelligenceSource(user.id,'house_document',documentId);
    ...
    recordHausmeisterDocumentUpload(user.id,{documentId,name:title,question:description,reply});
    revalidatePath('/app');revalidatePath('/app/hausmeister');revalidatePath('/app/documents');revalidatePath('/notifications');
    redirect(`/app/hausmeister?document=${documentId}`);
  }
