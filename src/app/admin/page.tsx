...
import { EHAppHeader, EHMetricsBar, EHRecordList, EHScope, EHStatus, EHWorkSection, EHWorkspaceGrid, type EHRecordEntry } from '@/design-system';
...
export default async function Admin({searchParams}:{searchParams:Promise<Record<string,string>>}){
  ...
  return <EHScope app><main className="admin-page"><EHAppHeader eyebrow="Betriebsverwaltung" title="Einfach Hausen · Admin" text="Vertragspartner, Qualitätsstandards, Leads und Servicefälle." actions={<><Link href="/admin/crm" className="btn primary"><Database size={16}/>Leads &amp; CRM</Link><form action={adminLogoutAction}><button className="btn ghost"><LogOut size={16}/>Abmelden</button></form></>} />{sp.error&&<div className="alert error" role="alert">{sp.error}</div>}<EHMetricsBar label="Verwaltung" items={[
