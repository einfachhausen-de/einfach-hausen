...
function statusLabel(status: string): { label: string; tone: 'warn' | 'info' | 'ok' | 'neutral' } {
  switch (status) {
    ...
    case 'done': return { label: 'Erledigt', tone: 'ok' };
...
export default async function Dashboard() {
  ...
  const recentHistory = db.prepare(`SELECT id,title,status,updated_at FROM jobs WHERE homeowner_id=? ORDER BY datetime(updated_at) DESC LIMIT 5`).all(user.id) as { id: number; title: string; status: string; updated_at: string }[];

  const profileFields = [!!(user.first_name && user.last_name), !!(user as { phone?: string }).phone, !!profile?.address, !!profile?.postcode];
      ...
      rail={
        <>
          ...
          <div className="eh-werkbank-karte">
            ...
            <div className="eh-werkbank-row"><span>Profil vollständig</span><span>{profilePct}% · {profileFilled} von {profileFields.length}</span></div>
            <div className="eh-werkbank-row"><span>Offene Vorgänge</span><span>{jobsCount}</span></div>
            <div className="eh-werkbank-row"><span>Dokumente</span><span>{documentCount}</span></div>
            {profilePct < 100 && <Link href="/app/profile" className="eh-werkbank-go">Profil vervollständigen →</Link>}
          </div>
          <div className="eh-werkbank-karte">
            <h4>Hilfe</h4>
            <p className={styles.railHelp}>Dein Hausmanager beantwortet Fragen zu Vorgängen, Terminen und Unterlagen.</p>
            <Link href="/app/hausmanager" className="eh-werkbank-go">Hausmanager fragen →</Link>
          </div>
        </>
      }
    >
      <div className={styles.dashRoot}>
        <header className={styles.dashHeader}>
          ...
          <div className={styles.dashHeaderActions}>
            <Link className={styles.dashPrimaryAction} href="/app/hausmeister"><Plus size={18} /> Neues Anliegen</Link>
          </div>
        </header>
        ...
        <section className={styles.overviewSection} aria-labelledby="overview-title">
          <p id="overview-title" className={styles.quickLabel}>Mein Zuhause im Überblick</p>
          <div className={`${styles.quickGrid} ${styles.quickGridThree}`}>
            <Link href="/app/jobs" className={styles.quickCard}>
              ...
              <small>{jobsCount === 0 ? 'Keine aktiven Aufträge.' : `${jobsCount} in Bearbeitung.`}</small>
            ...
            <Link href="/app/jobs" className={styles.quickCard}>
              <div className={styles.overviewCardTop}>
                ...
                <span className={styles.overviewCount} data-tone={offersCount > 0 ? 'terra' : undefined}>{offersCount}</span>
              </div>
              <strong>Angebote</strong>
              <small>{offersCount === 0 ? 'Keine offenen Angebote.' : `${offersCount} warten auf Entscheidung.`}</small>
            ...
            <Link href="/app/partners" className={styles.quickCard}>
              ...
              <small>{contactsCount === 0 ? 'Noch keine Partner.' : `${contactsCount} Partner.`}</small>
        ...
        <section className={styles.sectionCard} aria-labelledby="history-title">
          <header className={styles.sectionCardHeader}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span className={styles.itemIcon}><History size={18} /></span>
              <h2 id="history-title">Haus-Historie</h2>
            </div>
          ...
          {recentHistory.length > 0 ? (
                  ...
                  <li key={job.id}>
                    <Link href={`/app/jobs/${job.id}`} className={styles.contentsLink}>
                      ...
                      <span className={styles.itemMain}>
                        <b>{job.title}</b>
                        <small>{shortDay(job.updated_at)} · Nr. {job.id}</small>
                      </span>
                      <span className={styles.itemMeta}>
                        <span className={`${styles.pill} ${meta.tone === 'warn' ? styles.pillWarn : meta.tone === 'info' ? styles.pillInfo : ''}`}>{meta.label}</span>
            ...
            <div className={styles.emptyState}>
              <span className={styles.emptyIcon}><History size={22} /></span>
              <p>Noch keine Historie. Erledigte Aufträge erscheinen hier automatisch.</p>
              <Link href="/app/hausmeister">Ersten Auftrag starten</Link>
            </div>
          )}
        </section>
      </div>
