...
/** Beteiligter des Termins. Ohne hinterlegten Betrieb ist der Auftrag der Anker. */
const partnerLabel = (row: Appointment) => row.business_name || 'Wird noch zugeteilt';
...
export default async function Calendar({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}) {
  ...
  // Die zwei Kennzahlen zaehlen ueber die ganze Akte, nicht nur ueber die
  // gerade sichtbare Ansicht.
  ...
  return <WerkbankRahmen role="homeowner" active="/app/calendar" rail={<>
    ...
    <div className="eh-werkbank-kennzahlen">
      <EHMetricsBar label="Termine" items={[
        ...
        {id:'bestaetigt',label:'Bestätigt',value:upcomingConfirmed,hint:upcomingOpen > 0 ? `${upcomingOpen} noch offen` : 'alle bestätigt'},
      ]} />
    </div>
