...
import { BottomNav, isNavActive, providerNav } from './bottom-nav';
...
export async function AppShell({ role, active, children, title, subtitle, breadcrumbs }: { role:'homeowner'|'provider'; active:string; children:React.ReactNode; title?:string; subtitle?:string; breadcrumbs?:readonly Crumb[] }) {
  ...
  const mobileMenu = pro ? (
    <details className="mobile-menu">
      ...
      <nav className="mobile-menu-panel" aria-label="Hauptnavigation">{providerNav.map(([href,Icon,label])=><Link key={href} href={href} className={isNavActive(active,href)?'active':''} aria-current={isNavActive(active,href)?'page':undefined}><Icon size={18}/><span>{label}</span></Link>)}</nav>
    </details>
  ) : (
  ...
  return <EHScope app><EHWorkspaceFrame homeHref={pro?"/pro":"/app"}
    ...
    navigation={pro
      ? providerNav.map(([href,Icon,label])=><EHWorkspaceNavItem key={href} href={href} active={isNavActive(active,href)} icon={<Icon size={22}/>}>{label}</EHWorkspaceNavItem>)
    ...
    bottomNav={<BottomNav role={role} active={active}/>}>{breadcrumbs&&breadcrumbs.length>0&&<Breadcrumbs trail={breadcrumbs}/>}{children}</EHWorkspaceFrame></EHScope>;
}
