...
const STEPS = [
  ...
  { icon: MousePointerClick, title: 'Mit einem Klick wechseln', text: 'Wir kündigen deinen alten Vertrag für dich.' },
] as const;
    ...
    <section id="tarife" className="scroll-mt-24 bg-cream py-20 lg:py-28">
      <Container className="grid items-center gap-14 lg:grid-cols-2">
        <div className="order-2 lg:order-1">
          <div className="rounded-[2rem] bg-white p-5 shadow-[0_30px_60px_-30px_rgba(11,31,36,0.35)] sm:p-7">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-hairline pb-5">
              ...
              <span className="rounded-full bg-lime-soft px-3 py-1 text-xs font-bold text-save">Beispielrechnung</span>
            </div>
            ...
            <div className="mt-5 flex items-center justify-between rounded-2xl border border-dashed border-coral/40 bg-coral-soft px-4 py-3 text-sm">
              <span>
                <span className="block text-xs text-body">Dein aktueller Tarif</span>
                <strong>Grundversorgung</strong>
              </span>
              <span className="text-right">
                <strong className="block text-coral">1.482 € / Jahr</strong>
                <span className="text-xs text-body">123,50 € / Monat</span>
              </span>
            </div>
            ...
            <ul className="mt-3 flex flex-col gap-3">
              {OFFERS.map((offer) => (
                ...
                >
                  {offer.best && (
                    <span className="absolute -top-3 left-4 rounded-full bg-ink px-3 py-0.5 text-[11px] font-bold text-lime">
                      Bester Preis
                    </span>
                  )}
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-body">{offer.detail}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-display text-lg font-bold">{offer.price}</p>
                      <p className="text-xs font-semibold text-save">spart {offer.save}</p>
                    </div>
                  </div>
                  {offer.best && (
                    <div className="mt-3 flex h-10 items-center justify-center gap-2 rounded-full bg-lime text-sm font-bold text-ink" aria-hidden="true">
                      <FileSignature className="size-4" />
                      Jetzt wechseln – wir kündigen für dich
                    </div>
                  )}
                </li>
              ))}
        ...
        <div className="order-1 flex flex-col gap-8 lg:order-2">
          <SectionHeading
            ...
            title="Wechseln in 3 Minuten. Kündigen machen wir."
