...
import {type CitationItem, CitationList, CitationStack} from "@/components/ui/streaming-response-utils/citations";
import {EASE_OUT, SPRING_PRESS, SPRING_SWAP} from "@/components/ui/streaming-response-utils/ease";
import {cn} from "cn";
...
function ResponseAction({
  ...
  return (
    <motion.button
      ...
      aria-label={label}
      title={label}
      aria-pressed={label === "Hilfreich" || label === "Nicht hilfreich" ? isOn : undefined}
      onClick={onClick}
        ...
        {shouldShowActions ? (
          ...
          >
            <div className={cn("flex items-center gap-0.5", actionsClassName)}>
              ...
              {complete ? (
                <>
                  <ResponseAction label="Hilfreich" isOn={currentFeedback === "up"} onClick={() => setFeedback("up")}>
                    <ThumbsUp size={14} />
                  </ResponseAction>
                  <ResponseAction label="Nicht hilfreich" isOn={currentFeedback === "down"} onClick={() => setFeedback("down")}>
                    <ThumbsDown size={14} />
                  </ResponseAction>
                </>
              ) : null}
            ...
            {hasSources && currentSourcesOpen ? (
              <div id={sourcesContentId} className="mt-2 rounded-xl bg-muted p-2">
