...
Aktueller Stand (verifiziert 2026-09-23): **Repo-HEAD `0a3dc2b` (arena/01a0c66d-einfach-hausen)** — Hausmanager KI-Chat auf ChatGPT-Optik gebracht (PromptBox 28px Pill `+`/`Tools`/`Mic`/`↑`, Verlauf `History`-Head, streaming Quellen `CitationStack`), rechter Bereich bei `kiOffen` fixiert (`position:sticky top:58px height:calc(100vh-58px)` → `assistantPanel flex:1 height:100%`), Launcher bei offen versteckt, fetter 3px Fokus-Rahmen komplett entfernt (`assistantPromptBox:focus-within` nur noch `1px line`). Belege: `0a3dc2b`, `df1af3b`, `015374d`; Gates: `design:check EH_DESIGN_CONSISTENT`, `build` grün, `design:seal EH_DESIGN_SEALED`. Historisch: ses_f40d79 2026-09-20 (`3e8ecd9`) + T-0131 Convergence etc. bleibt gültig. Neue Arbeit nur aus Acceptance-Fehler als Remediation-Task.
