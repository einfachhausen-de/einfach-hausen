...
Aktueller Stand (verifiziert 2026-09-23): **Zweig `arena/01a0cb62-einfach-hausen`** — KI-Chat funktional fertiggestellt: (1) Haupt-`main` (`6adbcc9`, umbezogene Historie) ist per Merge mit unvereinbaren Historien zusammengeführt — Doku-Upload/OCR-Pipeline, File-Archiv-Tabellen und Agent-Doku bleiben erhalten; das Header-Menü folgt nach Betreiber-Urteil wieder dem Bereichs-Untermenü (Sub-Flyout, Guard: `scripts/header-menu.test.mjs`); (2) Chat-Tool-Buttons sind serverseitig verdrahtet: `POST /api/ki` akzeptiert `tool` aus strikter Allowlist (`create_job`, `compare_tariffs`, `compare_quotes`, `search_house`, `create_report`), unbekannte ID = 400; explizite Auswahl umgeht Laya/Jev; (3) neue Capabilities `search_house` (bündelt bestehende Tenant-sichere Lesefunktionen: Dokumente/OCR, Hausdaten, Verträge, Anlagen, Wartungen, Ansprechpartner) und `create_report` (erst serverseitig begrenzte eigene Daten, dann Generativ-Pfad mit BYOK/Quota/Refund, Transparenz über Einträge+Zeichen); (4) Plus-Button öffnet „Foto machen“ (bestehender privater Medienpfad, `/api/ki` erhält nur den Pfad) und „Datei hochladen“ (bestehende Hausakte-Pipeline: private Ablage, OCR/Laya-Klassifizierung, Chat-Rückmeldung); (5) Nachrichtenvorschläge: seitenbezogene Start-Chips (max. 3, ohne extra DB-Abfragen) und regelbasierte Follow-ups im `done`-Frame. Belege: Merges `ae74287`; Gates: `test:ai` 57/57, `design:check EH_DESIGN_CONSISTENT`, `tsc` grün, `eslint` 0 Fehler, `build` grün; Browser-Gate-Ebenen (a11y/visual/e2e) + `backup-retention` (vorbestehend rot in der Sandbox, siehe NEXT_AGENT) laufen auf der OCI-VM. Historisch: ses_f40d79 2026-09-20 (`3e8ecd9`) + T-0131 Convergence etc. bleibt gültig. Neue Arbeit nur aus Acceptance-Fehler als Remediation-Task.
...
## App-Rahmen · Stand `0a3dc2b` (2026-09-23)
...
- 2026-09-23 (`0a3dc2b`): **Hausmanager KI-Chat ChatGPT-Optik** — `EHAssistant` mit `Plus/SlidersHorizontal/Mic/ArrowUp` + 5-Tools Popover, `imagePreview`, `auto-resize 160px`, Verlauf `History`-Head, `EHActivity` live, `assistantPromptBox` 28px `bg-white shadow`, `assistantMessage` User-Bubble rechts / Assistant flach, `streaming-response` Token-konform; **rechter Rail fixiert** (`wb-rail[data-ki]{position:sticky top:58px height:calc(100vh-58px)}` + `assistantPanel{flex:1 height:100%}`), **Launcher versteckt** wenn `panel && offen`, **fetter Fokus-Rahmen entfernt** (alle `outline:3px`/`box-shadow:0 0 0 3px` → `none`, nur `1px line` bleibt). Gates: `design:check EH_DESIGN_CONSISTENT`, `build` ok, `EH_DESIGN_SEALED`.
...
Gemeinsamer Assistentenpfad für API und Hausmanager mit eigentümergebundenen Lese-Tools, Laya-Entscheidungen, Jev nur bei Überlast/Transportfehler und DeepSeek/BYOK ausschließlich hinter dem bestehenden generativen Kontingent. Architektur, Grenzen, Tests und Installation: [AI_ROUTER.md](docs/AI_ROUTER.md).
